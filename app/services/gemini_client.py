...
def _build_analysis_prompt(product: str | ProductUnderstanding, evidence: ProviderEvidence) -> str:
    ...
    return (
        "Extract structured supplier/source evidence for the VORA benchmark. Write every "
        "human-readable explanation, note, warning, match rationale, and summary in French; "
        "preserve official product titles, supplier names, brands, and URLs exactly. "
        ...
        "If the unit is barrel, ton, carton, or another bulk unit, mark the offer as "
        "non-comparable unless the evidence explicitly provides a safe conversion to the "
        "requested package. Never use a non-comparable offer for landed cost or margin. "
        ...
        "For retail prices, require a known package volume for comparable calculations. "
        "Keep an observed price with unknown volume only as non-comparable evidence; it "
        "must not influence retail min/max/average, market reference, landed-cost margin, "
...
def _build_client_analysis_prompt(
    ...
    return (
        ...
        "All client-facing human-readable output MUST be written entirely in professional "
        "French. Do not return English sentences, English explanations, or bilingual prose. "
        "Return product_description in French. Populate every localized_analysis field with "
        "French text; for backward-compatible *_en fields, repeat the same French text rather "
        "than translating it into English. Preserve official product names, brands, supplier "
        "names, and URLs exactly as source evidence when required. Do not mix languages inside "
        "the output fields product_description_en, product_description_fr, price_analysis_en/fr, "
        "market_analysis_en/fr, and business_reading_en/fr; these field names are retained for "
        "schema compatibility. Avoid generic filler such as 'insufficient data' on its own; "
        "say exactly which evidence is missing. "
        "any client-facing field. Every paragraph must stay specific to the requested product "
        "and cite the actual signals available in the evidence such as product name, source-backed "
        "price ranges, MOQ, seller count, landed-cost assumptions, or the exact missing market. "
        "Avoid generic filler such as 'données insuffisantes' or 'prix non disponible' on its own. "
        "If evidence is weak, say exactly which evidence is missing and what was still found for "
        "this product. "
        ...
        "recommendation_reason, and warnings. Put assumptions and risks inside "
        "profitability_estimate, not at the top level. The sourcing offer "
